...
Telegram возвращает `409 Conflict` если несколько процессов одновременно используют один токен. Процессы накапливаются между сессиями, и скрипт может называться по-разному — ищи **по рабочей директории**, а не по имени.
...
Убить все процессы, у которых рабочая директория совпадает с `/Users/.../mirit-assistant`:

```bash
kill <PID1> <PID2> ... 2>/dev/null
sleep 4  # дать Telegram освободить сессию
```
